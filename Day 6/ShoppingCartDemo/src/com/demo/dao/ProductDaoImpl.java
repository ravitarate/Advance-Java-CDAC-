...
public class ProductDaoImpl implements ProductDao{
	...
	static PreparedStatement prodselect,selectedprod;
	static {
		conn=DBUtil.getMyconnection();
		try {
			
			prodselect=conn.prepareStatement("select * from prodcat where catid=?");
			selectedprod=conn.prepareStatement("select * from prodcat where pid in ?");
	...
	public List<Product> findManyByCid(String[] parr) {
	    ...
	    for (int i = 1; i <= parr.length; i++) {
			...
			ResultSet rs=selectedprod.executeQuery();
			List<Product> pList=new ArrayList<>();
			while(rs.next()) {
				Product product=new Product(rs.getInt(1),rs.getString(2),rs.getInt(3),rs.getDouble(4),rs.getInt(5));
				pList.add(product);
